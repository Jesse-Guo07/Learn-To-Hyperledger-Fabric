## Fabric 测试网络------network.sh

> 路径：fabric-samples/test-network
...
#
# prepending $PWD/../bin to PATH to ensure we are picking up the correct binaries ---------》就是让加入环境变量不然很多运行不了
...
# This function is called when you bring a network down 清除现在所有的容器
...
function checkPrereqs() {
  ...
  #检查peer的二进制文件和一些配置文件是否存在（应该是core.yaml）所以一开始需要配置一下环境变量
...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    set -x #set是把下面命令打在屏幕，-x是开启+x是关闭
    cryptogen generate --config=./organizations/cryptogen/crypto-config-org1.yaml --output="organizations"
    res=$? #$?运行命令结束的代码，成功运行返回0
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
